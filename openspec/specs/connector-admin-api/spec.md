...
Phase 2 admin connector routes SHALL be reachable without authentication for local single-tenant development; production hardening is deferred.

#### Scenario: Local admin CRUD without auth

- **WHEN** a client calls admin connector routes on localhost without credentials
- **THEN** CRUD and Test operations succeed per validation rules
